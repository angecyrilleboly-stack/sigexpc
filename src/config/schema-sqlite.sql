-- ============================================================================
--  SIGEXPC - Schéma SQLite (alternative sans serveur, Node.js 24 natif)
--  Équivalent au schema.sql MySQL mais avec la syntaxe SQLite.
-- ============================================================================
...
CREATE TABLE IF NOT EXISTS auto_ecoles (
  ...
  statut TEXT DEFAULT 'actif',
  created_at TEXT DEFAULT (datetime('now','localtime')),
  FOREIGN KEY (id_region) REFERENCES directions_regionales(id) ON DELETE CASCADE
);
...
CREATE TABLE IF NOT EXISTS auto_ecoles_staff (
  ...
  date TEXT,
  created_at TEXT DEFAULT (datetime('now','localtime')),
  FOREIGN KEY (id_ae) REFERENCES auto_ecoles(id) ON DELETE CASCADE
);
...
CREATE TABLE IF NOT EXISTS candidats (
  ...
  created_at TEXT DEFAULT (datetime('now','localtime')),
  FOREIGN KEY (id_autoecole) REFERENCES auto_ecoles(id) ON DELETE CASCADE
);
...
CREATE TABLE IF NOT EXISTS inscriptions_examens (
  ...
  FOREIGN KEY (id_examen) REFERENCES examens_programmes(id) ON DELETE CASCADE
);
...
INSERT OR IGNORE INTO parametres_abonnement (id, montant, duree_jours) VALUES (1, 200, 30);
